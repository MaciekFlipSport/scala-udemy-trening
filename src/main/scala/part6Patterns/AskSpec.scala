...
class AskSpec extends TestKit(ActorSystem("AskSpec")) with ImplicitSender with WordSpecLike with BeforeAndAfterAll{
  ...
  "An authenticator" should {
    authenticatiorTestSuite(Props[AuthManager])
  }
  "An  piped authenticator" should {
    authenticatiorTestSuite(Props[PipedAuthManager])
  }

  def authenticatiorTestSuite(props: Props) = {
    import AuthManager._
    "fail to authenticate a non-registerd user" in {
      val authManager = system.actorOf(props)
      authManager ! Authenticate("Andrzej", "rtjvm")
      expectMsg(AuthFailure(AUTH_FAILURE_NOT_FOUND))
    }

    "fail to authenticate if invalid password" in {
      val authManager = system.actorOf(props)
      authManager ! RegisterUser("Jurek", "rtjvm")
      authManager ! Authenticate("Jurek", "zlehaslo")
      expectMsg(AuthFailure(AUTH_FAILURE_PASSWORD_INCORRECT))
    }

    "sucessfully authenticate a registered user" in {
      val authManager = system.actorOf(props)
...
object AskSpec {
  ...
  case class Write(key: String, values: String)
  ...
  class AuthManager extends Actor with ActorLogging {
    ...
    def handleAuthentication(username: String, password: String) = {
      ...
      future.onComplete {
        ...
        // NEVER CALL METHODS ON THE ACTOR INSANCE OR ACCESS MUTABLE STATE IN ONCOMPLITE
        //avoid closing over the actor instance or mutable state
        //originalSender in .onComplete method not sender() !!!!!!!
