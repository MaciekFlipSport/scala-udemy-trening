...
class FSMSpec extends TestKit(ActorSystem("FSMSpec"))
  ...
  def runTestSuite(props: Props)  = {
    ...
    "throw a timeout if I don't inster money" in {
      val vendingMachine = system.actorOf(props)
      vendingMachine ! Initialize(Map("coke" -> 10), Map("coke" -> 1 ))
    ...
    "deliver a product if I insert all the money" in {
      val vendingMachine = system.actorOf(props)
      vendingMachine ! Initialize(Map("coke" -> 10), Map("coke" -> 3))
      vendingMachine ! RequestProduct("coke")
      expectMsg(Instruction("Please insert 3 dollars"))
    ...
    def operational(inventory: Map[String, Int], prices: Map[String, Int]): Receive = {
      case RequestProduct(product) => inventory.get(product) match {
        ...
        case Some(_) =>
          ...
          context.become(waitForMoney(inventory, prices, product, 0, startReceivingMoneyTimeoutSchedule, sender()))
      }
    }
      ...
      case ReceiveMoney(amount) =>
        ...
        if (money + amount >= price) {
          ...
          context.become(waitForMoney(
            ...
            startReceivingMoneyTimeoutSchedule, // I need to set the timeout again
            requester))
        }
    }

    def startReceivingMoneyTimeoutSchedule = context.system.scheduler.scheduleOnce(1 second) {
      self ! ReceiveMoneyTimeout
    }
  }

  // step 1 - define states and the dat of the actor
  trait VendingState
