...
class TestProbeSpec extends TestKit(ActorSystem("TestProbeSpec"))
  ...
  "A master actor" should{
    ...
    "send the work to the slave actor" in {
      ...
      slave.reply(WorkComplited(3, testActor))
    ...
    "aggregate data correctly" in {
      ...
      slave.receiveWhile() {
        case SlaveWork(`workloadString`, `testActor`) => slave.reply(WorkComplited(3, testActor))
      }
      expectMsg(Report(3))
      expectMsg(Report(6))
    }
...
object TestProbeSpec {
  ...
  -slave processes the work and replies the master
  -master aggregates the result
  master sends the total count to the original requester
   */
  case class Work(text: String)
  case class SlaveWork(text: String, originalRequester: ActorRef)
  case class WorkComplited(count: Int, originalRequester: ActorRef)
  ...
  class Master extends Actor {
    ...
    def online(slaveRef: ActorRef, totalWordCount: Int): Receive = {
      ...
      case WorkComplited(count, originalRequester) =>
